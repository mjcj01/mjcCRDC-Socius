...
cp_rd_ggplot <- ggplot(cp_rr_rd_all %>% filter(type %in% c("bl_wh_rd", "hi_wh_rd", "ai_wh_rd", "as_wh_rd")), 
                       aes(x = YEAR, y = value, color = legend_text)) +
  geom_line(size = 1, lineend = "round") +
  guides(color = guide_legend(title = "Racial Group", byrow = TRUE)) +
  scale_color_manual(values = three_colors,
...
oss_rd_ggplot <- ggplot(oss_rr_rd_all %>% filter(type %in% c("bl_wh_rd", "hi_wh_rd", "ai_wh_rd", "as_wh_rd")), aes(x = YEAR, y = value, color = legend_text)) +
  geom_line(size = 1, lineend = "round") +
  guides(color = guide_legend(title = "Racial Group", byrow = TRUE)) +
  scale_color_manual(values = three_colors,
                     labels = function(x) str_wrap(x, width = 20)) +
...
### Combined layout
all_states <- plot_grid(plot_grid(oss_all_ggplot + theme(legend.position = "hidden"),
...
all_states_rr_rd <- plot_grid(plot_grid(oss_all_ggplot + theme(legend.position = "hidden"),
                                          cp_all_ggplot + theme(legend.position = "hidden"),
                                          racial_group_legend,
                                          ncol = 3,
                                          rel_widths = c(1, 1, 0.5)),
                                plot_grid(oss_rr_ggplot + theme(legend.position = "hidden",
                                          ...
                                          ncol = 3,
                                          rel_widths = c(1, 1, 0.5)),
                                plot_grid(oss_rd_ggplot + theme(legend.position = "hidden"),
                                          cp_rd_ggplot + theme(legend.position = "hidden"),
                                          rd_legend,
                                          ncol = 3,
                                          rel_widths = c(1, 1, 0.5)),
                                nrow = 3)
